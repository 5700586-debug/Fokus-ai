...
-- ``flush=True`` deploy qilingandan (PR #16, ``9387203``) KEYIN --
va ``LATENCY_PROBE`` dalilini Render'da topish uchun aniq UTC
boshlanish/tugash vaqtini va har bir buyruqning send-to-reply vaqtini
UCHALA joyga (stdout, ``::notice::`` annotation, ``$GITHUB_STEP_
SUMMARY``) yozadi:
...
run ``34168777808``da (commit ``b93f6b4``) isbotlangan versiyasi --
ssenariy mantig'i QAYTA LOYIHALANMAGAN, faqat UTC vaqt belgilarini
uchala joyga chiqarish va har bir buyruq vaqtini step summary'ga
yozish qo'shilgan.
...
def main() -> None:
    ...
    print(f"::notice::STARTED_AT_UTC={started}")
    ...
    print(f"::notice::FINISHED_AT_UTC={finished}")
