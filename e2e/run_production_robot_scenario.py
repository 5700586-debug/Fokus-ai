...
-- PostgreSQL connection pooling deploy qilingandan (PR #17,
``8bd2e83``) KEYIN -- va tezlik dalilini Render'siz ham (job log'ga
kirish shart bo'lmasdan) ko'rish uchun HAR BIR buyruq vaqtini,
STARTED_AT_UTC/FINISHED_AT_UTC bilan birga, UCHALA joyga (stdout,
``::notice::`` annotation, ``$GITHUB_STEP_SUMMARY``) yozadi:
...
run ``34173694313``da (commit ``62c0c8c``) isbotlangan versiyasi --
ssenariy mantig'i QAYTA LOYIHALANMAGAN, faqat HAR BIR buyruq vaqtini
ham ``::notice::`` orqali chiqarish qo'shilgan (avvalgi versiya faqat
STARTED/FINISHED UTC'ni notice qilardi, buyruq vaqtlari faqat stdout/
step summary'da edi).
...
# STEP 3.7 (proven run): hech qachon bu matnlardan biri ham
...
# Har bir buyruqning (label, elapsed_seconds) juftligi -- ``main()``
# oxirida ``$GITHUB_STEP_SUMMARY``ga yozish uchun.
...
def _record_timing(label: str, elapsed: float) -> None:
    ...
    # STEP 3.2: har bir buyruq vaqti ham job log'siz ko'rinishi uchun
    # ``::notice::`` orqali ham chiqariladi (avvalgi run faqat STARTED/
    # FINISHED UTC'ni shu yo'l bilan chiqargan edi).
...
async def _run(config: dict[str, str]) -> bool:
    ...
    try:
        async with client.conversation(bot_entity, timeout=_REPLY_TIMEOUT_SECONDS) as conv:
            ...
            # Bir yo'la ham pool'ni "isitadi" (birinchi haqiqiy DB
            # so'rovi shu yerda).
...
def _write_step_summary(started: str, finished: str, result: str) -> None:
    summary_path = os.getenv("GITHUB_STEP_SUMMARY")
    if not summary_path:
        return
    lines = [
        "## Production robot post-pool latency run",
        "",
...
def main() -> None:
    ...
    _write_step_summary(started, finished, result)
