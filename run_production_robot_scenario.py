...
-- ``/xarid`` batch price-history tuzatishi (PR #19, ``35005a2``)
...
FINISHED_AT_UTC bilan birga, ``::notice::`` GitHub annotation orqali
chiqaradi:
...
Bu skript ``run_production_robot_scenario.py``ning muvaffaqiyatli run
``34241424182``da (commit ``ba6128d``) isbotlangan versiyasi --
ssenariy mantig'i QAYTA LOYIHALANMAGAN. Faqat FARQI: ``scenario_total``
alohida ``::notice::``si OLIB TASHLANGAN -- oldingi run'da bu 11-chi
notice bo'lib, sahifa faqat 10 tasini ko'rsatgani uchun kesilib qolgan
edi. Endi ANIQ 10ta notice: 1 STARTED_AT_UTC + 8 qadam vaqti +
1 FINISHED_AT_UTC.
...
# Hech qachon bu matnlardan biri ham ko'rinmasin (bot javobida ham,
# skriptning o'z stdout/exception matnida ham).
...
def _write_step_summary(started: str, finished: str, result: str) -> None:
    ...
    if not summary_path:
        return
    lines = [
        "## Production robot post-xarid-batch-fix latency run",
        "",
