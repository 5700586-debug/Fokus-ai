...
| Funksiya | Holat | Fayl | Izoh |
|---|---|---|---|
| Tonggi (08:00) va tungi (21:00) rasmli salom — "Friendly phase" | production (test) | `services/saturn_group.py` (`send_morning_image_message`/`send_night_image_message`), `services/saturn_image.py`, `services/saturn_content.py` | 1080×1080 PNG, dastur orqali chizilgan (AI rasmga matn yozdirilmagan). Moliyaviy ma'lumot, KPI, jarima — YO'Q. Matn: sarlavha + bitta qisqa (≤120 belgi) tekshirilgan maslahat (35 tonggi + 31 tungi bank, AI ixtiyoriy ravishda qayta ifodalaydi, natija tekshiriladi). Yoqish/o'chirish: `/setrule saturn.morning_image_enabled\|night_image_enabled 0` |
| Tonggi rasmdagi ob-havo belgisi | qisman | `providers/weather_provider.py` (`OpenMeteoWeatherProvider`) | `WEATHER_PROVIDER_ENABLED=true` bo'lsa Open-Meteo (bepul, kalitsiz) ishlatiladi, aks holda ob-havo bloki butunlay yashiriladi (xato emas). Shahar/koordinata `SATURN_WEATHER_CITY`/`SATURN_WEATHER_LAT`/`SATURN_WEATHER_LON` orqali (standart: Qo'qon) |
...
| Qayta ishga tushganda/parallel chaqiruvda dublikat post yubormaslik | production (test), testlangan | Matnli postlar: `services/notifications.send_once`. Rasmli tonggi/tungi post: RESERVE-FIRST (`try_reserve`/`mark_sent`/`release_reservation`) — qimmat ishdan (AI/ob-havo/rasm) OLDIN atomik band qilinadi, shuning uchun ikkita parallel chaqiruv ham xato bermaydi | `tests/test_saturn_group_service.py` |
| Foydali ma'lumot (tip) takrorlanmasligi | production (test), testlangan | `_pick_tip`, `saturn_posts_log` | Oxirgi 7 kunlik tip bilan solishtiriladi (mustaqil, eski tip-bank — tonggi/tungi rasm maslahati alohida, oxirgi 30 kunlik banklardan) |
| Eski matnli tonggi salom / kechqurungi moliyaviy "Kun yakuni" | **saqlanган, lekin endi avtomatik yuborilmaydi** | `send_morning_message`/`send_evening_message` (`services/saturn_group.py`) | Xodimlar guruhiga ENDI avtomatik ketmaydi (yuqoridagi rasmli xabar bilan almashtirildi) — funksiya o'chirilmadi, faqat qo'lda `/saturntest evening` orqali (kelajakda boshqa/rahbar guruhida qayta ishlatilishi mumkinligi uchun) |
| Kunlik savdo raqamlari (reja/haqiqiy/cheklar/o'rtacha chek) — kunlik dashboard posti (12:00/16:00) | **rejalashtirilgan** | `providers/sales_data_provider.py` | O'zgarmadi — bu alohida, kun ichidagi dashboard posti (yuqoridagi tonggi/tungi salomdan farqli), hali `NullSalesDataProvider` bilan ishlaydi |
...
| C. Saturn umumiy guruhida Fokus AI (asosiy bot, test bot emas) | Tonggi/tungi rasmli post — **kod tayyor** (2026-08, `feature/saturn-daily-greetings`): moliyaviy raqamsiz, "Friendly phase". Qolgan qismi: iliq/o'zini tanishtiruvchi kirish xabari va tabrik/yutuqqa kontekstga mos munosabat bildirish — HALI QURILMAGAN | `saturn_group_bot.py`, `services/saturn_group.py`, `services/saturn_image.py` | **qisman** (kod tayyor, faqat TEST botda sinaladi — asosiy botga chiqarish alohida, Founder tasdig'i bilan bosqich) |
