...
izolyatsiyalangan E2E test (``Sinovchi``) ssenariysini ishga tushiradi
-- shared FSM/PostgreSQL o'qishlarni qisqartirish (PR #18, ``4d5f4a4``)
deploy qilingandan KEYIN -- va tezlik dalilini job log'ga kirish shart
bo'lmasdan ko'rish uchun HAR BIR buyruq vaqtini, STARTED_AT_UTC/
FINISHED_AT_UTC va umumiy ssenariy vaqti bilan birga, ``::notice::``
GitHub annotation orqali chiqaradi:
...
Bu skript ``e2e/run_production_robot_scenario.py``ning muvaffaqiyatli
run ``34223282248``da (commit ``8e5c00a``) isbotlangan versiyasi --
ssenariy mantig'i QAYTA LOYIHALANMAGAN, faqat umumiy ssenariy vaqtini
(``scenario_total``) ham alohida ``::notice::`` sifatida chiqarish
qo'shilgan (avvalgi versiyada faqat STARTED/FINISHED UTC va har bir
QADAM vaqti notice qilingan edi, umumiy vaqt esa faqat ikkalasi orasi
sifatida hisoblab olinishi kerak edi).
...
Render ilova loglari bu skriptdan O'QILMAYDI -- faqat robot/skript
darajasidagi ko'rinadigan dalil (qarang oxiridagi ``RENDER_LOG_CHECK``
chiqishi). Bu skript ichki DB/Telegram vaqtlarini robot elapsed
vaqtidan TAXMIN QILMAYDI -- faqat haqiqiy send-to-reply vaqtini
o'lchaydi.
"""
...
# STEP 3.6 (proven run): hech qachon bu matnlardan biri ham
# ko'rinmasin (bot javobida ham, skriptning o'z stdout/exception
# matnida ham).
...
# Har bir buyruqning (label, elapsed_seconds) juftligi.
...
def _record_timing(label: str, elapsed: float) -> None:
    ...
    # Har bir buyruq vaqti ham job log'siz ko'rinishi uchun ``::notice::``
    # orqali ham chiqariladi.
    print(f"::notice title=E2E STEP TIMING::{label}={elapsed:.2f}s{warn}")
...
async def _run(config: dict[str, str]) -> bool:
    ...
    try:
        ...
        # Muvaffaqiyatsizlikda bir martalik tozalashga urinish.
        try:
            async with client.conversation(bot_entity, timeout=_REPLY_TIMEOUT_SECONDS) as conv:
...
def _write_step_summary(started: str, finished: str, result: str, total_elapsed: float) -> None:
    summary_path = os.getenv("GITHUB_STEP_SUMMARY")
    if not summary_path:
        return
    lines = [
        "## Production robot post-FSM-optimization latency run",
        "",
        f"- STARTED_AT_UTC={started}",
        f"- FINISHED_AT_UTC={finished}",
        f"- SCENARIO_RESULT={result}",
        f"- scenario_total={total_elapsed:.2f}s",
        "",
    ...
    try:
        ...
        print(f"STEP_SUMMARY_WRITE_FAILED={error!r}")
...
def main() -> None:
    ...
    print(f"::notice title=E2E STARTED_AT_UTC::{started}")

    scenario_start_monotonic = time.monotonic()
    passed = asyncio.run(_run(config))
    scenario_total = time.monotonic() - scenario_start_monotonic
    ...
    print(f"FINISHED_AT_UTC={finished}")
    print(f"::notice title=E2E FINISHED_AT_UTC::{finished}")
    print(f"SCENARIO_TOTAL_SECONDS={scenario_total:.2f}")
    print(f"::notice title=E2E STEP TIMING::scenario_total={scenario_total:.2f}s")
    _write_step_summary(started, finished, result, scenario_total)
